...
use OC\Core\Sharing\GroupShareRecipientType;
...
class GroupShareRecipientTypeTest extends TestCase {
	...
	public function testGetRecipientValues(): void {
		$this->assertEquals(['group1'], $this->recipientType->getRecipientValues($this->user1, null));
	}
	...
	public function testSearchRecipients(): void {
		...
		$generateResult = static fn (string $groupId): ShareRecipientSearchResult => new ShareRecipientSearchResult(
			$groupId,
			self::DISPLAY_NAMES[$groupId],
			null,
			null,
			null,
		);

		$this->assertEquals(array_map($generateResult(...), ['group1', 'group2', 'group3']), $this->recipientType->searchRecipients('group', 3, 0));
		$this->assertEquals(array_map($generateResult(...), ['group1']), $this->recipientType->searchRecipients('group', 1, 0));
		$this->assertEquals(array_map($generateResult(...), ['group2', 'group3']), $this->recipientType->searchRecipients('group', 3, 1));
		$this->assertEquals(array_map($generateResult(...), ['group2']), $this->recipientType->searchRecipients('group', 1, 1));
	}
}
